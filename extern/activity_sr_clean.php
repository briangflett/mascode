  ...
  public function run()
  {
    ...
    if (!empty($a_results)) {
      ...
      foreach ($a_results as $activity) {
        ...
        $client_sql = $wpdb->prepare("SELECT id FROM civicrm_contact WHERE external_identifier = %s", $activity->ClientID);
        $client_id = $wpdb->get_var($client_sql);
        $ext_client_rep_id = strval(intval($activity->ClientRepID) + 1000000);
        ...
        if (!empty($activity->RequestID)) {
          $sr_sql = $wpdb->prepare("SELECT id FROM civicrm_case WHERE subject = %s", $activity->RequestID);
        ...
        // Get the project
        if (!empty($activity->ProjectID)) {
          $p_sql = $wpdb->prepare("SELECT id FROM civicrm_case WHERE subject = %s", $activity->ProjectID);
        ...
        $update_results = $wpdb->update(
          'bgf_dataload_tActivity',  // Table name
          array(
          ...
          array('ActivityID' => $activity->ActivityID)  // WHERE clause
        );

        // echo '<br> Updating Request ID: ' . $activity->RequestID . ' Results: ' . $update_results . '<br>';
        ...
        if ($row_count % $check_interval == 0) {
          if ((microtime(true) - $start_time) >= $timeout_limit) {
            ...
            $url = site_url('sites/all/modules/civicrm/extern/masdemo/activity_sr_clean.php?last_id=' . urlencode($last_id));
