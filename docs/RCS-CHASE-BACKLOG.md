...
4. Re-read the status. If it is not "Request RCS", set it back to
   "Request RCS" and abort the whole batch, loudly.
```
...
**Step 4 is not ceremony.** If a run dies between steps 2 and 3, the Service
Request is silently parked at "Ongoing": it drops out of the "Request RCS" queue
the coordinator works from, and it gets no chase either. That is a live client
case made worse, invisibly.

Note what step 4 is and is not. Step 1 has already established the case is at
"Request RCS", so there is no *prior* status to restore — the target is always
"Request RCS", and step 4 is a **verify-and-repair**, not a rollback to some
remembered value. It cannot help if the process is killed outright between the
two writes, which is why the batch should be small and the end state of every
case checked afterwards with the query below rather than assumed.
